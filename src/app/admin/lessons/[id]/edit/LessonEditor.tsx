...
import {
  ArrowLeft, Plus, Trash2, Save, Eye, EyeOff, GripVertical,
...
const BLOCK_META: Record<string, { label: string; icon: any; color: string }> = {
  ...
  IMAGE:    { label: "Şəkil",     icon: Image,         color: "bg-pink-100 text-pink-700" },
  QUIZ:     { label: "Quiz",      icon: HelpCircle,    color: "bg-orange-100 text-orange-700" },
  TASK:     { label: "Tapşırıq",  icon: ClipboardList, color: "bg-red-100 text-red-700" },
  DIVIDER:  { label: "Ayırıcı",   icon: Minus,         color: "bg-gray-100 text-gray-600" },
};
...
export default function LessonEditor({ lesson }: { lesson: any }) {
  const [meta, setMeta] = useState({
    ...
    shortDescriptionEn: lesson.shortDescriptionEn ?? "",
  ...
  async function addBlock(type: string) {
    ...
    const res = await fetch(`/api/lessons/${lesson.id}/blocks`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type, content: JSON.stringify(defaults[type] ?? {}), title: null }),
    });
  ...
  async function updateBlock(blockId: string, content: object, title?: string) {
    const block = blocks.find(b => b.id === blockId);
    if (!block) return;
    const updated = { ...block, content: JSON.stringify(content), title: title ?? block.title };
    setBlocks(blocks.map(b => b.id === blockId ? updated : b));
    await fetch(`/api/lessons/blocks/${blockId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ content: JSON.stringify(content), title: title ?? block.title }),
    });
  }
  ...
  async function moveBlock(blockId: string, dir: "up" | "down") {
    ...
    // Update orders
    await Promise.all([
      fetch(`/api/lessons/blocks/${newBlocks[idx].id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ order: idx }) }),
      fetch(`/api/lessons/blocks/${newBlocks[swap].id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ order: swap }) }),
    ]);
  }
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="font-bold text-gray-900">Dərs blokları ({blocks.length})</h2>
          ...
          {blocks.length === 0 && (
            <div className="card text-center text-muted py-10">
              Hələ blok yoxdur. Yuxarıdan blok əlavə edin.
            </div>
          )}

          {/* Blocks list */}
          {blocks.map((block, idx) => {
            const meta2 = BLOCK_META[block.type] ?? BLOCK_META.TEXT;
            const Icon = meta2.icon;
            const isActive = activeBlock === block.id;
            const c = parse(block);

            return (
              <div key={block.id} className={cn("card transition-all", isActive && "ring-2 ring-blue-400 shadow-md")}>
                {/* Block header */}
                <div className="flex items-center gap-2 mb-3 cursor-pointer" onClick={() => setActiveBlock(isActive ? null : block.id)}>
                  <GripVertical size={16} className="text-gray-300 shrink-0" />
                  <div className={`w-7 h-7 rounded-lg ${meta2.color} flex items-center justify-center shrink-0`}>
                    <Icon size={14} />
                  </div>
                  <span className="text-sm font-semibold text-gray-700 flex-1">{meta2.label}</span>
                  {block.title && <span className="text-xs text-muted truncate max-w-32">{block.title}</span>}
                  <div className="flex items-center gap-1 ml-auto">
                    <button onClick={e => { e.stopPropagation(); moveBlock(block.id, "up"); }} disabled={idx === 0} className="btn-ghost btn-sm py-1 px-1.5 disabled:opacity-30">
                      <ChevronUp size={14} />
                    </button>
                    <button onClick={e => { e.stopPropagation(); moveBlock(block.id, "down"); }} disabled={idx === blocks.length - 1} className="btn-ghost btn-sm py-1 px-1.5 disabled:opacity-30">
                      <ChevronDown size={14} />
                    </button>
                    <button onClick={e => { e.stopPropagation(); deleteBlock(block.id); }} className="btn-ghost btn-sm py-1 px-1.5 text-red-400 hover:text-red-600">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
                ...
                {isActive && (
                  <div className="border-t border-gray-100 pt-3 space-y-3">
                    <BlockEditor block={block} content={c} onChange={(newC, title) => updateBlock(block.id, newC, title)}
                ...
                {!isActive && (
                  <BlockPreview block={block} content={c} />
                )}
              </div>
            );
          })}
        ...
        <div className="space-y-4">
          <div className="card space-y-4 sticky top-4">
            ...
            <div>
              <label className="label">Qısa təsvir (AZ)</label>
              <textarea className="textarea h-16 text-sm" value={meta.shortDescription}
                onChange={e => setMeta({ ...meta, shortDescription: e.target.value })} />
            </div>
            <div>
              <label className="label">Müddət (dəq)</label>
              <input type="number" className="input text-sm" min={5} max={300}
                value={meta.estimatedMinutes} onChange={e => setMeta({ ...meta, estimatedMinutes: parseInt(e.target.value) })} />
            </div>

            <div className="space-y-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 rounded" checked={meta.published}
                  onChange={e => setMeta({ ...meta, published: e.target.checked })} />
                <span className="text-sm font-medium text-gray-700">Yayımlanıb</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 rounded" checked={meta.quizEnabled}
                  onChange={e => setMeta({ ...meta, quizEnabled: e.target.checked })} />
            ...
            {meta.quizEnabled && (
              <div>
                ...
                <input type="number" className="input text-sm" min={0} max={100}
                  value={meta.minQuizScore} onChange={e => setMeta({ ...meta, minQuizScore: parseInt(e.target.value) })} />
              </div>
            )}
...
function BlockPreview({ block, content: c }: { block: any; content: any }) {
  switch (block.type) {
    ...
    case "IMAGE":    return c.url ? <div className="text-sm text-muted">🖼 {c.caption || c.url}</div> : <div className="text-sm text-muted italic">Şəkil seçilməyib</div>;
...
// ─── Block editor (expanded state) ───────────────────────────────
function BlockEditor({ block, content: c, onChange, onImageUpload, uploadingImage }: {
  block: any; content: any; onChange: (c: object, title?: string) => void;
  onImageUpload: () => void; uploadingImage: boolean;
}) {
  switch (block.type) {
    case "HEADING":
      return (
        <div className="space-y-2">
          <div>
            <label className="label">Başlıq mətni</label>
            <input className="input" value={c.text ?? ""} onChange={e => onChange({ ...c, text: e.target.value })} />
          </div>
          <div>
            <label className="label">Səviyyə</label>
            <div className="flex gap-2">
              {[2, 3, 4].map(l => (
                <button key={l} onClick={() => onChange({ ...c, level: l })}
                  className={cn("btn-secondary btn-sm", c.level === l && "bg-blue-700 text-white border-blue-700")}>
                  H{l}
                </button>
              ))}
            </div>
    ...
    case "TEXT":
      return (
        <div className="space-y-2">
          <div>
            <label className="label">Başlıq (isteğe bağlı)</label>
            <input className="input" placeholder="Bölmə başlığı..." value={block.title ?? ""}
              onChange={e => onChange(c, e.target.value)} />
          </div>
          <div>
            <label className="label">Mətn</label>
            <textarea className="textarea h-32" value={c.body ?? ""}
              onChange={e => onChange({ ...c, body: e.target.value })} />
          </div>
          <div>
            <label className="label">Vurğu</label>
            <div className="flex gap-2">
              {(["normal", "info", "warning", "success"] as const).map(h => (
                <button key={h} onClick={() => onChange({ ...c, highlight: h })}
                  className={cn("btn-secondary btn-sm capitalize", c.highlight === h && "bg-blue-700 text-white border-blue-700")}>
                  {h === "normal" ? "Yox" : h === "info" ? "Məlumat" : h === "warning" ? "Xəbərdarlıq" : "Uğur"}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-2">
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {(["important", "tip", "warning", "remember"] as const).map(v => (
                <button key={v} onClick={() => onChange({ ...c, variant: v })}
                  className={cn("btn-secondary btn-sm", c.variant === v && "bg-blue-700 text-white border-blue-700")}>
                  {v === "important" ? "⚠️ Vacib" : v === "tip" ? "💡 Məsləhət" : v === "warning" ? "🔶 Diqqət" : "📌 Yadda saxla"}
                </button>
              ))}
            </div>
          </div>
          <div>
            <label className="label">Mətn</label>
            <textarea className="textarea h-24" value={c.body ?? ""}
              onChange={e => onChange({ ...c, body: e.target.value })} />
          </div>
        </div>
      );
      ...
      return (
        <div className="space-y-2">
          <div>
            ...
            <input className="input" placeholder="Nümunə başlığı..." value={block.title ?? ""}
              onChange={e => onChange(c, e.target.value)} />
          </div>
          <div>
            <label className="label">İzahat</label>
            <textarea className="textarea h-20" value={c.description ?? ""}
              onChange={e => onChange({ ...c, description: e.target.value })} />
          </div>
          <div>
            <label className="label">Əsas çıxarış</label>
            <input className="input" value={c.takeaway ?? ""}
              onChange={e => onChange({ ...c, takeaway: e.target.value })} />
          </div>
        </div>
      );
    ...
    case "TABLE":
      return (
        <div className="space-y-3">
          <div>
            <label className="label">Başlıq</label>
            <input className="input" placeholder="Cədvəl başlığı..." value={block.title ?? ""}
              onChange={e => onChange(c, e.target.value)} />
          </div>
          <div>
            <label className="label">Sütunlar (vergüllə ayır)</label>
            <input className="input" value={(c.headers ?? []).join(", ")}
              onChange={e => onChange({ ...c, headers: e.target.value.split(",").map((s: string) => s.trim()) })} />
          </div>
          <div>
            ...
            {(c.rows ?? []).map((row: string[], ri: number) => (
              <div key={ri} className="flex gap-2 mb-2">
                <input className="input text-sm flex-1" value={row.join(", ")}
                  onChange={e => {
                    const rows = [...(c.rows ?? [])];
                    rows[ri] = e.target.value.split(",").map((s: string) => s.trim());
                    onChange({ ...c, rows });
                  }} />
                <button onClick={() => {
                  const rows = (c.rows ?? []).filter((_: any, i: number) => i !== ri);
                  onChange({ ...c, rows });
                }} className="btn-ghost btn-sm text-red-400"><Trash2 size={13} /></button>
              </div>
            ))}
            <button onClick={() => onChange({ ...c, rows: [...(c.rows ?? []), Array(c.headers?.length ?? 2).fill("")] })}
              className="btn-secondary btn-sm mt-1"><Plus size={13} /> Cərgə əlavə et</button>
          </div>
        </div>
      );
    ...
    case "IMAGE":
      return (
        <div className="space-y-2">
          <div>
            <label className="label">Şəkil</label>
            <div className="flex gap-2">
              <input className="input flex-1 text-sm" placeholder="https://..." value={c.url ?? ""}
          ...
          <div>
            <label className="label">Başlıq</label>
            <input className="input" value={c.caption ?? ""} onChange={e => onChange({ ...c, caption: e.target.value })} />
          </div>
          <div>
            <label className="label">Hizalanma</label>
            <div className="flex gap-2">
              {(["left", "center", "right"] as const).map(a => (
                <button key={a} onClick={() => onChange({ ...c, alignment: a })}
                  className={cn("btn-secondary btn-sm", c.alignment === a && "bg-blue-700 text-white border-blue-700")}>
        ...
        <div className="space-y-4">
          {(c.questions ?? []).map((q: any, qi: number) => (
            <div key={q.id} className="bg-gray-50 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => onChange({ ...c, questions: c.questions.filter((_: any, i: number) => i !== qi) })}
                  className="btn-ghost btn-sm text-red-400"><Trash2 size={13} /></button>
              </div>
                ...
                {(q.options ?? []).map((opt: string, oi: number) => (
                  <div key={oi} className="flex items-center gap-2">
                    <button onClick={() => {
                      ...
                      onChange({ ...c, questions: qs });
                    }} className={cn("w-5 h-5 rounded-full border-2 shrink-0 transition-colors",
              ...
              <div>
                <label className="label text-xs">İzahat (isteğe bağlı)</label>
                <input className="input text-sm" placeholder="Düzgün cavabın izahatı..." value={q.explanation ?? ""}
                  onChange={e => {
                    const qs = [...c.questions];
          ...
          <button onClick={() => onChange({
            ...c, questions: [...(c.questions ?? []), {
              id: `q-${Date.now()}`, text: "Yeni sual?",
              options: ["Variant A", "Variant B", "Variant C", "Variant D"],
              correctIndex: 0, explanation: ""
            }]
          })} className="btn-secondary btn-sm w-full">
            <Plus size={13} /> Sual əlavə et
          </button>
        </div>
      );
    ...
    case "TASK":
      return (
        <div className="space-y-2">
          <div>
            <label className="label">Başlıq</label>
            <input className="input" placeholder="Tapşırıq başlığı..." value={block.title ?? ""}
              onChange={e => onChange(c, e.target.value)} />
          </div>
          <div>
            <label className="label">Şərtlər</label>
            <textarea className="textarea h-20" value={c.instructions ?? ""}
              onChange={e => onChange({ ...c, instructions: e.target.value })} />
          </div>
          <div>
            <label className="label">Placeholder</label>
            <input className="input" value={c.placeholder ?? ""}
              onChange={e => onChange({ ...c, placeholder: e.target.value })} />
          </div>
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" className="w-4 h-4 rounded" checked={c.required ?? true}
              onChange={e => onChange({ ...c, required: e.target.checked })} />
            <span className="text-sm font-medium text-gray-700">Məcburi tapşırıq</span>
          </label>
        </div>
      );
    ...
    default:
      return <div className="text-sm text-muted">Bu blok növü üçün redaktor yoxdur</div>;
  }
}
