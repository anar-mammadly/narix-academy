      ...
      <div className="space-y-4">
              ...
              <div className="border-t border-gray-100 pt-3 space-y-1">
                {mod.lessons.map((lesson, j) => (
                  <div key={lesson.id} className="flex items-center gap-3 py-1.5 px-2 rounded-lg hover:bg-gray-50 transition-colors">
                    <span className="text-xs text-muted w-8">{i + 1}.{j + 1}</span>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Link href={`/admin/lessons/${lesson.id}/edit`} className="btn-ghost btn-sm py-1 px-2 text-blue-600">
                        <Edit size={13} /> Düzəlt
                      </Link>
                    </div>
                  </div>
        ...
        {modules.length === 0 && (
          <div className="card text-center text-muted py-10">
            Hələ modul yoxdur. İlk modulu yarat!
          </div>
        )}
      </div>
    </div>
